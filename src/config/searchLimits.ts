...
export const DESKTOP_SEARCH_LIMITS: SearchLimitsConfig = {
  default: 25,      // Limite maior para desktop
  all: 5,          // 5 de cada tipo quando "tudo" está selecionado
  artist: 25,      // Mais artistas
  album: 10,       // 10 álbuns
  track: 25,       // Mais músicas
  playlist: 20,    // Mais playlists
  show: 15,        // Mais shows
  episode: 12,     // Mais episódios
  audiobook: 10,   // Mais audiobooks
}

// Função para obter o limite baseado no tipo de busca
export function getSearchLimit(types: string[], config: SearchLimitsConfig = SEARCH_LIMITS): number {
...
export function getLimitByType(type: string, config: SearchLimitsConfig = SEARCH_LIMITS): number {
  const limit = config[type as keyof SearchLimitsConfig] || config.default
...
export function getSearchLimitWithDevice(types: string[]): number {
  ...
  if (types.length === 1) {
    const type = types[0]
    return config[type as keyof SearchLimitsConfig] || config.default
  }
